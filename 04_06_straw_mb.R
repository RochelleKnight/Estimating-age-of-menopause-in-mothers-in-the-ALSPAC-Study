...
df_mb <- df_mb %>% mutate(exclude_mb = case_when(MB4610 == 1 | MB4660 == 1 | MB4670 == 1 ~ 1,
                                           TRUE~ NA_integer_))
...
df_mb <- df_mb %>% mutate(straw_mb = case_when(menopause_status == "reproductive" & period_history_mb == "last_3_months" ~ "reproductive",
                                         ...
                                         menopause_status == "menopause_transition" & period_history_mb == "more_12_months" ~ "post_menopause",
...
tmp <- df_mb %>% filter(menopause_status =="menopause_transition")  %>% select(aln,straw_mb,menopause_status,period_history_mb)
